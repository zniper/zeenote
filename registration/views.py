...
from simplenote.settings import RC_PRIVATE_KEY, SERVER_IP
...
def register(request, backend, success_url=None, form_class=None,
    ...
    **Template:**
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            # validate captcha
            rc_challenge = request.POST['recaptcha_challenge_field']
            rc_response = request.POST['recaptcha_response_field']
            rc_result = captcha.submit(rc_challenge, 
                                       rc_response, 
                                       RC_PRIVATE_KEY, 
                                       SERVER_IP)
            # continue only if captcha is valid
            if rc_result.is_valid:
                ...
                extra_context = {'invalid_captcha':'1'}
                logger.error("ReCaptcha Error: " + str(rc_result.error_code))
    else:
        form = form_class()
